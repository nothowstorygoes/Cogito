...
export default function Advanced() {
    ...
    const buttonBase = "rounded-3xl w-35 h-10 font-semibold hover:w-40 cursor-pointer transition-all duration-300";
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            await window.electron.invoke("save-file", { filePath, content: JSON.stringify(loggerData, null, 2) });
    ...
    return (
        <main className={`w-screen h-screen overflow-hidden flex flex-col transition-colors duration-300
            ${dark ? "bg-[#181825]" : "bg-[#D2D6EF]"}`}>
            <TitleBar />
            <div className="flex flex-col items-center h-full mt-10">
                <h1 className={`text-3xl font-bold mb-4 transition-colors duration-300 ${dark ? "text-[#D2D6EF]" : "text-[#6331c9]"}`}>Advanced Settings</h1>
                <p className={`text-md mb-4 w-96 text-center transition-colors duration-300 ${dark ? "text-[#D2D6EF]" : "text-[#6331c9]"}`}>
                    In this section you can import preexisting data or export a backup of your log.
                </p>
                <div className="flex flex-row items-center justify-between w-80 mb-10">
                    ...
                    <button
                        ...
                        disabled={exporting}
                    >{exporting ? "Exporting..." : "Export Log"}</button>
                </div>
                ...
                {/* Status Message */}
                {message && (
                    <div className={`mt-4 p-3 rounded-lg w-[96] text-center ${messageType === 'success'
                ...
                <button></button>
                <button
                    className={`${buttonBase} ${dark ? buttonDark : buttonLight} absolute top-130 right-10`}
