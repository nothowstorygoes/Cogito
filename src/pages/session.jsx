...
export default function Session() {
    ...
    const handleFinish = () => {
        ...
        // Optionally, also trigger close-session (resize and route)
    ...
    return (
        <main className="w-screen h-screen bg-[#D2D6EF] flex flex-col items-center justify-center">
            <TitleBar />
            <div className="text-3xl font-bold text-[#6331c9]">{formatTime(seconds)}</div>
                ...
                <div className="flex flex-row gap-x-3 mt-4 justify-center items-center">
                    {!running && (
                        <button className="cursor-pointer" onClick={handleStart}>
                            <svg width="40" height="40" viewBox="0 0 24 24" fill="#6331c9">
                                <polygon points="10,8 18,12 10,16" fill="#6331c9" />
                            </svg>
                        </button>
                    )}
                    ...
                    {running && (
                        <button className="cursor-pointer" onClick={handlePause}>
                            <svg width="40" height="40" viewBox="0 0 24 24" fill="#6331c9">
                                <rect x="8" y="8" width="3" height="8" fill="#6331c9" />
                                <rect x="13" y="8" width="3" height="8" fill="#6331c9" />
                            </svg>
                        </button>
                    )}
                    {/* Finish/Check Button */}
                    <button className="mb-1 cursor-pointer" onClick={handleFinish} disabled={seconds === 0}>
                        <svg width="40" height="40" viewBox="0 0 24 24" fill="#6331c9">
                            <polyline points="7,13 11,17 17,9" fill="none" stroke="#6331c9" strokeWidth="2" />
                        </svg>
                    </button>
                </div>
